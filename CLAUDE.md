...
- IMPORTANT: Never hardcode hex values inline — always use `AppColors.*` or `colors.*` (AdaptiveColors)
- IMPORTANT: Every view must support dark mode via `AdaptiveColors`
- IMPORTANT: Use `AppColors.primary` (`#7C6FF7`) as the main accent, NOT the navy blue from older brand guidelines
- Category-specific colors are defined in `availableCategoryColors` array for user-customizable categories
...
| `standard` | `easeInOut(duration: 0.3)`                     | State transitions       |
| `smooth`   | `spring(response: 0.4, dampingFraction: 0.8)`  | Smooth reveals          |
| `bouncy`   | `spring(response: 0.5, dampingFraction: 0.7)`  | Playful bounces         |

- Progress ring: `spring(response: 0.8, dampingFraction: 0.75)`
- Logo entrance: `spring(response: 0.6, dampingFraction: 0.7)` with 0.1s delay
...
- **REPS:** IRS Real Estate Professional Status — requires 750+ hours/year in real estate activities AND the 50% rule (RE hours ≥ 50% of total working hours)
